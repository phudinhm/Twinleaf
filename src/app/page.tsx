...
import { Leaf, ChevronUp, Download, Loader2 } from "lucide-react";
...
export default function Home() {
  ...
  const handleSpeak = useCallback((text: string, lang: string, id: number) => {
  ...
  const handleSearchNav = useCallback(
    (direction: "prev" | "next") => {
      ...
      const idx = chunks.findIndex((c) => c.id === targetId);
      if (idx >= 0) {
        virtualizer.scrollToIndex(idx, { align: "center", behavior: "smooth" });
        setActiveChunkId(targetId);
      }
    },
    [searchMatchIds, currentMatchIndex, chunks, virtualizer]
  );
  ...
  return (
    <main
      className="h-screen flex flex-col reader-transition overflow-hidden"
      style={{ backgroundColor: ts.bg }}
    >
      {/* ── Top Sticky Toolbar ── */}
      <header
        className="flex-shrink-0 backdrop-blur-md border-b px-4 py-2.5 z-30 reader-transition shadow-sm"
              ...
              return (
                ...
                >
                  <ParagraphPair
                    ...
                    isSpeaking={speakingId === chunk.id}
      ...
      <button
        onClick={scrollToTop}
        className="fixed bottom-6 right-6 w-11 h-11 rounded-full shadow-lg flex items-center justify-center transition hover:scale-110 active:scale-95 z-20 border reader-transition"
        style={{
